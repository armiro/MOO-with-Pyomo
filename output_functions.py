def hardness_removal_efficiency(model):
    intercept = 55.64
    linear_coefs = -8.78 * model.h_conc + 3.53 * model.s_conc + 12.28 * model.ph + 3.07 * model.time + \
                   3.09 * model.current
    ...
    quadratic_coefs = 2.07 * (model.h_conc ** 2) + 2.17 * (model.s_conc ** 2) - 0.27 * (model.ph ** 2) - \
                      3.42 * (model.time ** 2) - 2.72 * (model.current ** 2)
    return intercept + linear_coefs + interaction_coefs + quadratic_coefs


def silica_removal_efficiency(model):
    intercept = 96.05
    linear_coefs = -0.0358 * model.h_conc + 2.73 * model.s_conc + 0.49 * model.ph - 1.16 * model.time + \
                   0.53 * model.current
    ...
    quadratic_coefs = -0.19 * (model.h_conc ** 2) - 1.43 * (model.s_conc ** 2) - 0.28 * (model.ph ** 2) + \
                      0.6 * (model.time ** 2) - 0.16 * (model.current ** 2)
    return intercept + linear_coefs + interaction_coefs + quadratic_coefs


def operating_cost(model):
    intercept = 3.81
    linear_coefs = 0.12 * model.h_conc - 0.005 * model.s_conc - 0.034 * model.ph + 1.3 * model.time + \
                   1.4 * model.current
    ...
    quadratic_coefs = -0.03 * (model.h_conc ** 2) - 0.025 * (model.s_conc ** 2) - 0.14 * (model.ph ** 2) + \
                      0.052 * (model.time ** 2) - 0.28 * (model.current ** 2)
    return intercept + linear_coefs + interaction_coefs + quadratic_coefs
